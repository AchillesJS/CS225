...
class BinaryTree
{
  ...
  private:
     ...
     * @param isBST 
     */
    void isOrdered(const Node* subRoot, T& lastVisited, bool& isBST) const;

    /**
     * Private helper for printPaths().
     * @param subRoot The current node в рекурсии
     * @param currentPath 
     * @param paths Сборник всех путей
     */
    void printPaths(const Node* subRoot, vector<T>& currentPath,
