...
for (size_t i = 0; i < tra_.size(); ++i) {
    auto currentTraversal = tra_[i];
    auto itBegin = currentTraversal->begin();
    auto itEnd = currentTraversal->end();
    ...
    for (auto it = itBegin; it != itEnd; ++it) {
        ...
        int x = (*it).x;
        int y = (*it).y;
        HSLAPixel* origPixel = png_->getPixel(x, y);
        HSLAPixel newPixel = cp_[i]->getColor(x, y);
