...
Flower::Flower(const Vector2& center)
    : stem(
          Vector2(center.x(), center.y() + STEM_HEIGHT / 2),
          STEM_COLOR,
          STEM_WIDTH,
          STEM_HEIGHT
      ),
      pistil(
          Vector2(center.x(), center.y() - PISTIL_RADIUS / 2),
          PISTIL_COLOR,
          PISTIL_RADIUS
      ),
      leaf(
          ...
          Vector2((center.x() - (STEM_WIDTH / 2) + 1) - LEAF_WIDTH,
                  center.y() + (3 * STEM_HEIGHT / 4) - LEAF_HEIGHT),
          Vector2((center.x() - (STEM_WIDTH / 2) + 1) - (LEAF_WIDTH / 2),
                  center.y() + (3 * STEM_HEIGHT / 4))
      )
{ 
}
