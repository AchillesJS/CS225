...
void SCHashTable<K, V>::remove(K const& key)
{
     ...
     */
     size_t idx = hash(key, size);
    list<pair<K, V>>& bucket = table[idx];
...
void SCHashTable<K, V>::resizeTable()
{
     ...
     * @hint Use findPrime()!
     */
    size_t expandedSize = findPrime(size * 2);
    auto rehashed = new list<pair<K, V>>[expandedSize];
    for (size_t bucket = 0; bucket < size; ++bucket) {
        for (it = table[bucket].begin(); it != table[bucket].end(); ++it) {
            const auto& entry = *it;
            size_t newIdx = hash(entry.first, expandedSize);
            rehashed[newIdx].emplace_front(entry);
        }
    }
    table = rehashed;
    size = expandedSize;
